...
(function(t, u) {
    ...
    if (!t.seajs) {
        ...
        e.prototype.load = function() {
            if (!(this.status >= j.LOADING)) {
                ...
                for (var c = this._remain = b.length, a, d = 0; d < c; d++)
                    a = e.get(b[d]), a.status < j.LOADED ? a._waitings[this.uri] = (a._waitings[this.uri] || 0) + 1 : this._remain--;
                if (0 === this._remain)
                    this.onload();
                else {
                    for (var g = {}, d = 0; d < c; d++)
        ...
        e.prototype.onload = function() {
            ...
            for (c in b)
                if (b.hasOwnProperty(c) && (a = r[c], a._remain -= b[c], 0 === a._remain))
                    a.onload();
        ...
        f.config = function(b) {
            for (var c in b) {
                ...
                if (d && X(d))
                    ...
                    A(d) ? a = d.concat(a) : "base" === c && ("/" === a.slice( - 1) || (a += "/"), a = I(a)), h[c] = a
            }
            m("config", b);
            return f
        }
